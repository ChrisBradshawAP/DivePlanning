...
IF "%1"=="B:" GOTO BSTART
IF "%1"=="b:" GOTO BSTART
ECHO RE-BREATHER files to be copied to drive A:
ECHO press 'CTRL-C' to cancel
ECHO Insert a blank disc in drive A:,
PAUSE
IF EXIST A:*.* GOTO :NOTBLANK
ECHO Copying files, Please wait . . .
COPY rebrplan.exe A:\diveplan.exe   > NUL
...
reccount r
ECHO.
ECHO Re-Breather PRO-DIVE PLANNER copied to drive A:
ECHO.
GOTO :END
:NOTBLANK  
...
GOTO :END
:BSTART
ECHO RE-BREATHER files to be copied to drive B:
ECHO press 'CTRL-C' to cancel
ECHO Insert a blank disc in drive B:,
PAUSE
IF EXIST B:*.* GOTO :NOTBBLANK
ECHO Copying files, Please wait . . .
COPY rebrplan.exe B:\diveplan.exe   > NUL
...
reccount r
ECHO.
ECHO Re-Breather PRO-DIVE PLANNER copied to drive B:
ECHO.
GOTO :END
:NOTBBLANK  
