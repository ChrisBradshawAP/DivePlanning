...
Begin VB.Form Planprofile2 
   BackColor       =   &H00C0C0C0&
   Caption         =   "ProPlanner Library Dive Editor"
   ClientHeight    =   9375
   ClientLeft      =   165
   ClientTop       =   -1080
   ClientWidth     =   12585
   ...
   Begin VB.PictureBox Picture3 
      ...
      Begin VB.CommandButton cmdtimedown 
         ...
         Height          =   190
         Left            =   2040
         Picture         =   "Planprofile2.frx":E2E67
         Style           =   1  'Graphical
         TabIndex        =   52
         Top             =   1200
         Width           =   220
      ...
      Begin VB.CommandButton cmdtimeup 
         ...
         Height          =   190
         Left            =   1800
         Picture         =   "Planprofile2.frx":E3011
         Style           =   1  'Graphical
         TabIndex        =   51
         Top             =   1200
         Width           =   220
      ...
      Begin VB.CommandButton cmddepthdown 
         ...
         Height          =   195
         Left            =   1200
         Picture         =   "Planprofile2.frx":E31BB
         Style           =   1  'Graphical
         TabIndex        =   50
         Top             =   1200
         Width           =   220
      ...
      Begin VB.CommandButton cmddepthup 
         ...
         Height          =   190
         Left            =   960
         Picture         =   "Planprofile2.frx":E3365
         Style           =   1  'Graphical
         TabIndex        =   49
         Top             =   1200
         Width           =   220
      ...
      Begin VB.CommandButton Command4 
         ...
         Height          =   190
         Left            =   1560
         Picture         =   "Planprofile2.frx":E350F
         Style           =   1  'Graphical
         TabIndex        =   46
         Top             =   2400
         Width           =   220
      ...
      Begin VB.CommandButton CMDPPO2PLUS 
         ...
         Height          =   190
         Left            =   1320
         Picture         =   "Planprofile2.frx":E36B9
         Style           =   1  'Graphical
         TabIndex        =   45
         Top             =   2400
         Width           =   220
      ...
      Begin VB.Image Image1 
         ...
         Height          =   4215
         Left            =   0
         Picture         =   "Planprofile2.frx":E3863
         Stretch         =   -1  'True
         Top             =   0
         Width           =   5675
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame7 
         ...
         Begin VB.PictureBox Picture2 
            ...
            Height          =   2895
            Left            =   2400
            Picture         =   "Planprofile2.frx":1C0AA5
            ScaleHeight     =   2835
            ScaleWidth      =   3195
            TabIndex        =   24
            Top             =   120
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Label lblcylsize 
         ...
         BackColor       =   &H00FFFFFF&
         BackStyle       =   0  'Transparent
         Caption         =   "Surface Air Consumption SAC"
         BeginProperty Font 
            Name            =   "Microsoft Sans Serif"
            Size            =   8.25
            Charset         =   0
...
Dim DataChanged, ppo2changed, tempdepth, temptime, tempo2, temppo2, temphe, tempgasindex, tempcircuit As String
Dim zoom As Integer
Dim pan As Integer
...
Private Sub atmtext_KeyPress(KeyAscii As Integer)
   If KeyAscii = 13 Then
    If Val(atmtext.Text) < 400 Or Val(atmtext.Text) > 1000 Then
...
Private Sub cbogasindex_Change()
  ...
  If CDbl(txtdepth.Text) > CDbl(txtmaxd(CInt(Right(cbogasindex.Text, 1)))) Then
    'MsgBox "Depth too deep for gas - change gas or depth gas1" + CStr(Time)
    'MsgBox "Depth too deep for gas - change gas or depth gas2" + CStr(Time)
    'MsgBox "Depth too deep for gas - change gas or depth gas3" + CStr(Time)
    ...
    Exit Sub
  End If
  If MSFlexGrid3.Rows > 1 Then cmdmodify.Visible = True
...
validate_data
 ...
 If Option3.Value = True Then
    ...
    MSFlexGrid3.Col = 5
    If (Val(txtppo2v.Text) < (Val(lblo2.Caption) / 100) * ((Val(txtdepth) / 10) + 1)) Then
      MSFlexGrid3.Text = Format((Val(lblo2.Caption) / 100) * ((Val(txtdepth) / 10) + 1), "##0.00")
...
ans = MsgBox("Do you really want to delete the depth point(s)?", vbYesNo + vbCritical + vbDefaultButton2, "Comfirmation")
Select Case ans
Case vbYes
   removerecord
   cleargriddata2
...
checkgasindex
     If checkgasselected = True Then
        If Val(txtdepth) > 0 And Val(txtdepth) < 2001 Then
           txtdepth.Text = CStr(CDbl(txtdepth.Text) - (1# / feetormeter_factor))  'txtdepth = txtdepth - 1
           If Option4.Value = True Then
              txtppo2v = CStr(((CDbl(txtdepth) / 10#) + 1#) * (CDbl(lblo2.Caption) / 100))
              txtppo2v = Format(txtppo2v, "###.00")
           End If
        Else
           If Val(txtdepth) <= 0 Then
              txtdepth.Text = "10"
           Else
  ...
  'MsgBox Val(txtdepth)
     If checkgasselected = True Then
        If Val(txtdepth) >= 0 And Val(txtdepth) < 2000 Then
           txtdepth.Text = CStr(CDbl(txtdepth.Text) + (1# / feetormeter_factor))
           If Option4.Value = True Then
              txtppo2v = CStr(((CDbl(txtdepth) / 10#) + 1#) * (CDbl(lblo2.Caption) / 100))
              txtppo2v = Format(txtppo2v, "###.00")
           End If
...
Private Sub savemaxdepth()
  ...
  tempmaxdepth = 0
   ...
   For p = K To MSFlexGrid3.Rows - 1
      ...
      If Val(tempmaxdepth) <= Val(MaxDepth) Then
   ...
   While RS3.EOF = False
      RS3.Edit
      RS3!MaxDepth = Format(tempmaxdepth, "###0.0")
      RS3.Update
      RS3.MoveNext
   Wend
   SQL = "select * FROM seqdplist "
...
checkgasindex
   If checkgasselected = True Then
      If Val(txttime) > 0 And Val(xttime) < 9999 Then
         txttime = txttime - 1
      'Else
      '   txttime = "10"
      End If
...
For i = 0 To 9
  ...
  End If
Next i
Label21.Caption = "MOD " & feetormeter_shortstring
Label11.Caption = feetormeter_shortstring 'feetormeter_string
formstarted = True
datachangedstatus = False
txtdepthft_focus = 0
txtmaxd_focus = 0
...
Dim temptext6 As String
   ...
   Text2.Text = Text2.Text + comptext + vbCrLf
   comptext = lblcylsize(0).Caption + " " + txtcylcap.Text + vbCrLf + lblcylsize(1).Caption + " "
   If feetormeter_feeton = 1 Then
     comptext = comptext + txtbreathratecuft.Text
   Else
     comptext = comptext + txtbreathrate.Text
   End If
   comptext = comptext + " " + lblcylsize(3).Caption
   Text2.Text = Text2.Text + comptext + vbCrLf
   
   comptext = ""
   Text2.Text = Text2.Text + comptext + vbCrLf
    
    comptext = "Gas #" & vbTab & "O2" & vbTab & "He" & vbTab & "Depth" & vbTab & "PPO2" & vbTab & "Gas Used" & vbTab & vbTab & vbTab & lblcylsize(2).Caption
    Text2.Text = Text2.Text + comptext + vbCrLf
    comptext = ""
    For v = 0 To 9
      temptext = "Gas " + CStr(v)
      temptext2 = txtoxygen(v).Text
      temptext3 = txthelium(v).Text
      temptext4 = txtmaxdft(v).Text 'lbldepth(v).Caption
      temptext5 = txtppo2(v).Text
      temptext6 = CStr(Cbogasused(v).Text)
      comptext = temptext & vbTab & temptext2 & vbTab & temptext3 & vbTab & temptext4 & vbTab & temptext5 & vbTab & temptext6
      comptext = comptext & vbTab & vbTab + " " + gasusage(v).Caption
      If Len(gasusage(v).Caption) > 0 Then comptext = comptext + Left(lblcylsize(2).Caption, 3)
      Text2.Text = Text2.Text + comptext + vbCrLf
      comptext = ""
    Next v
    comptext = ""
...
'If decoresultgrid.CellBackColor = &H8000000F Then
...
If timer2buffer < 10 Then Exit Sub
 If depthcount > 0 Then
  ...
  If checkgasselected = True Then
        If Val(txtdepth) >= 0 And Val(txtdepth) < 2000 Then
           If depthcount = 1 Then txtdepth.Text = CStr(CDbl(txtdepth.Text) + (1# / feetormeter_factor))
           If depthcount = 2 Then txtdepth.Text = CStr(CDbl(txtdepth.Text) - (1# / feetormeter_factor))
           If Option4.Value = True Then
              txtppo2v = CStr(((CDbl(txtdepth) / 10#) + 1) * (CDbl(lblo2.Caption) / 100))
              txtppo2v = Format(txtppo2v, "###.00")
           End If
...
Private Sub txtdepth_Change()
 ...
 ' MsgBox Right(cbogasindex.Text, 1)
...
SendKeys "{END}"
  ...
  If CDbl(txtdepth.Text) > CDbl(txtmaxd(CInt(Right(cbogasindex.Text, 1)))) Then
    ...
    Exit Sub
  End If
  If Val(txtdepth) < 1 Or Val(txtdepth) > 2000 Then
     MSFlexGrid3.Col = 1
     txtdepth.Text = MSFlexGrid3.Text
     SendKeys "{END}"
     Exit Sub
...
Private Sub validateoxygen()
   If Val(txtoxygen(p).Text) >= 0 And Val(txtoxygen(p).Text) <= 100 And ((Val(txtoxygen(p).Text) + Val(txthelium(p).Text)) < 101) Then
     ...
     If Cbogasused(p).Text = "5 - Deco Closed Circuit" Then
'       If (Val(temptextpo2) <> Val(txtppo2(p).Text)) Then
...
Private Sub validateppo2()
 If Val(txtppo2(p).Text) >= 0.15 And Val(txtppo2(p).Text) < 2# Then
     tempgasindex = lblgasindex(p).Caption
     SQL = "SELECT * FROM dpmaingaslist "
     SQL = SQL & " where dpmainid = '" & tempserialno & "' "
     SQL = SQL & " and dpgasid = '" & tempgasindex & "' "
...
If KeyAscii = 13 Then
  If Val(txtmaxdft(Index).Text) < 0 Or Val(txtmaxdft(Index).Text) > 2000 Then
     MsgBox " Depth value out of range !"
  End If
End If
End Sub
...
SendKeys "{END}"
If Val(txttime) < 1 Or Val(txttime) > 2000 Then
     MSFlexGrid3.Col = 2
     txttime.Text = MSFlexGrid3.Text
     SendKeys "{END}"
     Exit Sub
...
'  MsgBox Plan_Gas_list_n2(1)
  ...
  For i = 0 To 9
    ...
    If barused(i + 1) < 1# Then gasusage(i).Visible = False Else gasusage(i).Visible = True
  Next
...
Private Sub safetytext_KeyPress(KeyAscii As Integer)
   If KeyAscii = 13 Then
    If Val(safetytext.Text) < 0 Or Val(safetytext.Text) > 50 Then
...
'Add by Goh on 08/11/2004 12:30pm
  ...
  For i = 1 To decoresultgrid.Rows - 1
    ...
    If IsNumeric(decoresultgrid.Text) = True Then
      'If CInt(decoresultgrid.Text) = row_count Then
        decoresultgrid.Col = 2
...
'  Picture1.Line (0, 0)-(x(1) * xscale, y(2) * yscale), vbBlue
  For i = 1 To row_count
     Picture1.Line (X(i - 1) * xscale, Y(i - 1) * yscale)-(X(i) * xscale, Y(i) * yscale), vbYellow
  Next i
  Frame8.Visible = True
...
gasusage(i).Caption = Format(barused(i + 1) * psiorbar_factor * CDbl(txtbreathrate.Text) / CDbl(txtcylcap.Text), "####") 'er_factor * CDbl(txtbreathrate.Text) / CDbl(txtcylcap.Text), "####")
