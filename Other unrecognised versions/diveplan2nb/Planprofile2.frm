...
Begin VB.Form Planprofile2 
   ...
   Begin VB.Frame Frame3 
      ...
      Top             =   3480
      Width           =   4575
      Begin VB.CommandButton Resetvhmx 
         Caption         =   "0 0 0"
         BeginProperty Font 
            Name            =   "Microsoft Sans Serif"
            Size            =   6
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Label Label34 
         ...
         Left            =   4450
         TabIndex        =   209
         ToolTipText     =   "Surface Air Comsumption in Bar/min for given cylinder size"
         Top             =   180
         Width           =   360
      End
      Begin VB.Label Label33 
         ...
         Left            =   4150
         TabIndex        =   197
         ToolTipText     =   "Water Capacity of cylinder in litres"
         Top             =   180
         Width           =   280
      End
   ...
   Begin VB.Menu mnuHelp 
      Caption         =   "&Help"
      Begin VB.Menu mnuPlanHelp 
         Caption         =   "Plan Help"
      End
      Begin VB.Menu mnuvgminfo 
         Caption         =   "VGM info"
      End
   End
End
...
Dim bn2max(16) As Double
Dim bhemax(16) As Double
    Dim nfrac As Double
...
Dim gf_1 As Double
Dim gf_2 As Double
Dim returnfunc As Double
Dim bmin_fast(17) As Double
Dim bmin_mid(17) As Double
Dim bmin_slow(17) As Double
Dim bmax_fast(17) As Double
Dim bmax_mid(17) As Double
Dim bmax_slow(17) As Double
Dim p1_point_default(17) As Double
Dim p1_pointsafe(17) As Double
Dim p1_pointmid(17) As Double
Dim p1_pointslow(17) As Double
Dim p1_distancedefault(17) As Double
Dim p1_distancesafe(17) As Double
Dim p1_distancemid(20) As Double
Dim p1_distanceslow(18) As Double
Dim p2_turnline_points00(17) As Double '(2)(2) =
Dim p2_turnline_points01(17) As Double '(2)(2) =
Dim p2_turnline_points10(17) As Double '(2)(2) =
Dim p2_turnline_points11(17) As Double '(2)(2) =
Dim p2_turnline_point1_safe(17) As Double
Dim p2_turnline_point1_fast(17) As Double
Dim p2_turnline_point1_mid(17) As Double
Dim p2_turnline_point2_safe(17) As Double
Dim p2_turnline_point2_fast(17) As Double
Dim p2_turnline_point2_mid(17) As Double
Dim gas_mixn2 As Double
Dim gas_mixhe As Double
Dim cbmin(4) As Double
Dim cbbb(4) As Double
Dim depthbot As Double
Dim timebot As Double
Dim tissueheliumbot(17) As Double
Dim tissuenitrogenbot(17) As Double
Dim ans_N2 As Double
Dim ans_He As Double
Dim ans_tot As Double
...
RS!Plandate = Now
RS3!PlanBy = vhmx_text(0).Text + ",a" + vhmx_text(1).Text + ",b" + vhmx_text(2).Text + ",c" + Label6.Caption
RS!divecategories = "1"
...
'float bmax()(2) =
'{{1.2298,1.04228},{1.2971,1.1634},{1.255048,1.13427},{1.1297,1.0428},{0.9929,0.9264},{0.9882,0.9323},{0.9999,0.9544},
'{0.9801,0.9408},
'{0.8709,0.83881},{0.8452,0.81596},{0.8162,0.78803},{0.7593,0.7326},{0.7552,0.7269},{0.7239,0.69562},{0.71847,0.68966}
',{0.71239,0.6839}}
  bn2max(0) = 1.2298
  bn2max(1) = 1.2971
  bn2max(2) = 1.255048
...
Next K
End Sub
Private Sub printtogrid5lite(first_stop As Integer)
If rate > 0.01 Then
  decoresultgridlite.Rows = decoresultgridlite.Rows + 1
  decoresultgridlite.Row = decoresultgridlite.Rows - 1
  decoresultgridlite.Col = 8
...
If first_stop = 1 Then
  If vsegment_vtime < 1# Then
    decoresultgridlite.Text = Format(1#, "###0.0")
  Else
    decoresultgridlite.Text = Format(CInt(vsegment_vtime + 0.4999), "###0.0")
  End If
Else
  decoresultgridlite.Text = Format(vsegment_vtime, "###0.0")
End If
...
If first_stop Then
  totalmins = CInt(tempgridlitemins) ' + CInt(tempdurationlitemins)
  templeftsec = 0 'decoresultgridlite.Text = CDbl(totalmins) & ":" & "00" 'templeftsec
Else
  totalmins = CInt(tempgridlitemins) + CInt(tempdurationlitemins)
  decoresultgridlite.Text = CDbl(totalmins) & ":" & templeftsec
End If
...
'            vdepth = Plan_Depth(Planpoint)
          ...
          Else
            ...
            ending_vdepth = Plan_Depth(Planpoint)
            If ending_vdepth > depthbot Then depthbot = ending_vdepth
            If Plan_OpenClosed(Planpoint) = 1 Then
              SetPoint = Plan_PPo2(Planpoint)
            Else
              SetPoint = 0#
...
'            t1print8 cstr(rate)
        ...
        If current_vdepth > depthbot Then depthbot = current_vdepth
        
    Loop 'Next Planpoint
    For i = 1 To 16
       tissueheliumbot(i - 1) = vhelium_vpressure(i) / 10#
       tissuenitrogenbot(i - 1) = vnitrogen_vpressure(i) / 10#
    Next i
    timebot = run_vtime_end_of_vsegment
    '=======================================================================
...
'                  gf_1 = (vhmx_ptissue - (vdeco_vstop_vdepth + barometric_vpressure)) / (vhmx_tol_pressure_bnorm - (vdeco_vstop_vdepth + barometric_vpressure))
                ...
                vdeco_vstop_vdepth = next_vstop
                last_run_vtime = run_vtime
                If vdeco_vstop_vdepth <= 0 Then '< 6 Then
...
For i = 2 To 16
    If ascent_vceiling_vdepth > compartment_ascent_vceiling(i) Then
      i = i
    Else
      i = i
      'gf_1 = (vhmx_ptissue - (vdeco_vstop_vdepth + barometric_vpressure)) / (vhmx_tol_pressure_bnorm - (vdeco_vstop_vdepth + barometric_vpressure))
          ...
          vhmx_first_stop = i
      gf_1 = (vnitrogen_vpressure(i) + vhelium_vpressure(i) - (compartment_ascent_vceiling(i) + barometric_vpressure)) / (vhmx_tol_pressure_bnorm - (compartment_ascent_vceiling(i) + barometric_vpressure))
...
L720:
    ...
    If temp_vsegment_vtime >= 10000 Then
      i = 1
    End If
    If (vdeco_vceiling_vdepth > next_vstop) And temp_vsegment_vtime < 1000 Then
...
'  MsgBox Plan_Gas_list_n2(1)
  ...
  If no_deco_found > 0 Then
    ...
    Picture1.Cls
    MsgBox "No Decompression possible with current gas and safety settings!!" + vbCrLf + "Revise values to obtain viable decompression."
  End If
  For i = 0 To 9
...
'  Picture1.Line (0, 0)-(x(1) * xscale, y(2) * yscale), vbBlue
  ...
  For i = 1 To row_count
     decoresultgrid.Row = i
     num = CInt(decoresultgrid.Text) - 1
     If InStr(1, num, "0") Then C = vbWhite
     If InStr(1, num, "1") Then C = vbYellow
     If InStr(1, num, "2") Then C = vbCyan
     If InStr(1, num, "3") Then C = &H808080
     If InStr(1, num, "4") Then C = vbGreen
     If InStr(1, num, "5") Then C = vbMagenta
     If InStr(1, num, "6") Then C = vbBlue
     If InStr(1, num, "7") Then C = &HFF80FF
     If InStr(1, num, "8") Then C = &H8080FF
     If InStr(1, num, "9") Then C = vbRed
...
vhmx_safe_factor = 1# + 0.01 * CDbl(vhmx_text(3).Text)
 ...
 '   maxd24 = maxdstart30 + maxdnorm80 ' //150.0) maxd24=150.0
  ...
  If i = 0 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 1 Then If amultfreal > 1.85 Then amultfreal = 1.85
  If i = 2 Then If amultfreal > 1.6 Then amultfreal = 1.6
  If i = 3 Then If amultfreal > 1.5 Then amultfreal = 1.5
  If i = 4 Then If amultfreal > 1.4 Then amultfreal = 1.4
  If i = 5 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 6 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 7 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 8 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 9 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 10 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 11 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 12 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 13 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 14 Then If amultfreal > 1.99 Then amultfreal = 1.99
  If i = 15 Then If amultfreal > 1.99 Then amultfreal = 1.99
...
Private Sub vhmx_text_Change(index As Integer)
  ...
  If CInt(vhmx_text(index).Text) >= 1 Then vhmx_text(index).BackColor = vbGreen
...
'void solve_lineqn(float* point1,float* point2,float* rtn) //Solve y = mx + c given 2 points
'void coefficient_calc(float minf, float pinf, float bturn()(2),float* rtn) //Solve y = gamma*tanh(beta(x-alpha))+delta given 2 points
'float inv_tanh(float ii) //Calculate arctanh(x)
'float b_calc(float time,float depth,int gas,int comp)//Main calculations done here
'float partial_tissue_pressure(float pa,float p0,float time,int compartment,int gas)


'float bmin()(2) =
'{{0.505,0.4245},{0.6514, 0.5747}, {0.7222, 0.6527}, {0.7825, 0.7223}, {0.8126, 0.7582}, {0.8434,0.7957}, {0.8693,0.8279}, {0.8910, 0.8553},
'{0.9092,0.8757}, {0.9222, 0.8903},{0.9319, 0.8997}, {0.9403, 0.9073}, {0.9477, 0.9122}, {0.9544,0.9171}, {0.9602,0.9217}, {0.9653,0.9267}}

'float bmax()(2) =
'{{1.2298,1.04228},{1.2971,1.1634},{1.255048,1.13427},{1.1297,1.0428},{0.9929,0.9264},{0.9882,0.9323},{0.9999,0.9544},{0.9801,0.9408},
'{0.8709,0.83881},{0.8452,0.81596},{0.8162,0.78803},{0.7593,0.7326},{0.7552,0.7269},{0.7239,0.69562},{0.71847,0.68966},{0.71239,0.6839}}

'float half_time()(2) =
'{{4.0,1.51},{8.0,3.02},{12.5,4.72},{18.5,6.99},{27.0,10.21},{38.3,14.48},{54.3,20.53},{77.0,29.11},
'{109.0,41.2},{146.0,55.19},{187.0,70.69},{239.0,90.34},{305.0,115.29},{390.0,147.42},{498.0,188.24},{635.0,240.03}}

'float gas_mix() = {0.40,0.15}
'float tissue()(2) = {{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},
'    {0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0},{0.79,0.0}}

Function jpb_main(arg_c As Integer)  ' //pp_t
Dim i
Dim p0 As Double
Dim pp_tissue0 As Double

'//      fputs("Enter your depth (m): ", stdout)
'//      fputs("Enter your time (min): ", stdout)
'//      fputs("Enter your gas mix (N2): ", stdout)
'//      fputs("Enter your gas mix (He): ", stdout)
'//      fputs("Enter your gas type (N2/He): ", stdout)
'//      fputs("Enter your compartment number (1-16): ", stdout)
'//      fputs("Enter your safety factor (0.85-1.4): ", stdout)
'//      fputs("Enter your fast safety factor (0.65-1.4): ", stdout)
'//      fputs("Enter your mid safety factor (0.65-1.4): ", stdout)
'//      fputs("Enter your slow factor (0.65-1.4): ", stdout)

 bmin_fast(0) = 0.35 / 0.4
 bmin_fast(1) = 0.3 / 0.4
 bmin_fast(2) = 0.25 / 0.4
 bmin_fast(3) = 0.2 / 0.4
 bmin_fast(4) = 0.15 / 0.4
 bmin_fast(5) = 0.1 / 0.4
 bmin_fast(6) = 0.05 / 0.4
 bmin_fast(7) = 0#
 bmin_fast(8) = 0#
 bmin_fast(9) = 0#
 bmin_fast(10) = 0#
 ...
 bmin_mid(9) = 0.35 / 0.4
 bmin_mid(10) = 0.3 / 0.4
 bmin_mid(11) = 0.25 / 0.4
 bmin_mid(12) = 0.2 / 0.4
 bmin_mid(13) = 0.15 / 0.4
 bmin_mid(14) = 0.1 / 0.4
 bmin_mid(15) = 0#
  ...
  bmin_slow(7) = 0
  bmin_slow(8) = 0
  bmin_slow(9) = 0.05 / 0.4
  bmin_slow(10) = 0.1 / 0.4
  bmin_slow(11) = 0.15 / 0.4
  bmin_slow(12) = 0.2 / 0.4
  bmin_slow(13) = 0.25 / 0.4
  bmin_slow(14) = 0.3 / 0.4
  bmin_slow(15) = 0.35 / 0.4

  bmax_fast(0) = 0.3391 / 0.4
  bmax_fast(1) = 0.2785 / 0.4
  bmax_fast(2) = 0
  bmax_fast(3) = 0.1666 / 0.35
  bmax_fast(4) = 0.121 / 0.35
  bmax_fast(5) = 0.08134 / 0.35
  bmax_fast(6) = 0.03813 / 0.35
  bmax_fast(7) = 0
  bmax_fast(8) = 0
  bmax_fast(9) = 0
  bmax_fast(10) = 0
  ...
  bmax_mid(1) = 0
  bmax_mid(2) = 0
  bmax_mid(3) = 0.1666 / 0.35
  bmax_mid(4) = 0.204 / 0.35
  bmax_mid(5) = 0.24414 / 0.35
  bmax_mid(6) = 0.28 / 0.35
  bmax_mid(7) = 0.318 / 0.35
  bmax_mid(8) = 0.31 / 0.35
  bmax_mid(9) = 0.269 / 0.35
  bmax_mid(10) = 0.227 / 0.35
  bmax_mid(11) = 0.1842 / 0.35
  bmax_mid(12) = 0.14 / 0.35
  bmax_mid(13) = 0.0946 / 0.35
  bmax_mid(14) = 0.0398 / 0.35
  bmax_mid(15) = 0#
  ...
  bmax_slow(6) = 0
  bmax_slow(7) = 0
  bmax_slow(8) = 0.044 / 0.35
  bmax_slow(9) = 0.0897 / 0.35
  bmax_slow(10) = 0.1364 / 0.35
  bmax_slow(11) = 0.1842 / 0.35
  bmax_slow(12) = 0.233 / 0.35
  bmax_slow(13) = 0.2888 / 0.35
  bmax_slow(14) = 0.33 / 0.35
  bmax_slow(15) = 0.389 / 0.35
  
  p1_point_default(0) = 30#
  p1_point_default(1) = 30#
  p1_point_default(2) = 30#
  p1_point_default(3) = 30#
  p1_point_default(4) = 30#
  p1_point_default(5) = 30#
  p1_point_default(6) = 30#
  p1_point_default(7) = 80#
  p1_point_default(8) = -90#
  p1_point_default(9) = -30#
  p1_point_default(10) = -30#
  p1_point_default(11) = 20#
  p1_point_default(12) = 10#
  p1_point_default(13) = 20#
  p1_point_default(14) = 30#
  p1_point_default(15) = 15#
  ...
  p1_pointsafe(6) = 0#
  p1_pointsafe(7) = 0#
  p1_pointsafe(8) = 5# / 3# / 0.1
  p1_pointsafe(9) = 10# / 3# / 0.1
  p1_pointsafe(10) = 3# / 0.1
  p1_pointsafe(11) = -1.75 / 0.1
  p1_pointsafe(12) = -5# / 0.1
  p1_pointsafe(13) = -1.75 / 0.1
  p1_pointsafe(14) = -1# / 0.1
  p1_pointsafe(15) = -3# / 0.1
  ...
  p1_pointmid(6) = 0#
  p1_pointmid(7) = 0#
  p1_pointmid(8) = 160 / 90# / 0.12
  p1_pointmid(9) = 8 / 3# / 0.12
  p1_pointmid(10) = 7 / 3# / 0.12
  p1_pointmid(11) = -0.75 / 0.12
  p1_pointmid(12) = -3 / 0.16
  p1_pointmid(13) = -0.75 / 0.2
  p1_pointmid(14) = -1 / 3# / 0.35
  p1_pointmid(15) = 0#
  ...
  p1_pointslow(8) = 0#
  p1_pointslow(9) = 0#
  p1_pointslow(10) = 7 / 3# / 0.2
  p1_pointslow(11) = -1.75 / 0.2
  p1_pointslow(12) = -6 / 0.2
  p1_pointslow(13) = -2 / 0.15
  p1_pointslow(14) = -4 / 3# / 0.15
  p1_pointslow(15) = -3 / 0.1
  
'//First 8 compartments of p1_distancedefault = 200 to ensure that p1_point_turn = 30
'//for all compartments later in calculation
  p1_distancedefault(0) = 200#
  p1_distancedefault(1) = 200#
  p1_distancedefault(2) = 200#
  p1_distancedefault(3) = 200#
  p1_distancedefault(4) = 200#
  p1_distancedefault(5) = 200#
  p1_distancedefault(6) = 200#
  p1_distancedefault(7) = 200#
  p1_distancedefault(8) = 75#
  p1_distancedefault(9) = 50#
  p1_distancedefault(10) = 45#
  p1_distancedefault(11) = 40#
  p1_distancedefault(12) = 50#
  p1_distancedefault(13) = 60#
  p1_distancedefault(14) = 40#
  p1_distancedefault(15) = 60#
  ...
  p1_distancesafe(6) = 0#
  p1_distancesafe(7) = 0#
  p1_distancesafe(8) = 2 / 3# / 0.1
  p1_distancesafe(9) = 3 / 5# / 0.1
  p1_distancesafe(10) = 0.5 / 0.1
  p1_distancesafe(11) = 3 / 8# / 0.1
  p1_distancesafe(12) = 0.5 / 0.1
  p1_distancesafe(13) = 7 / 12# / 0.1
  p1_distancesafe(14) = 3 / 8# / 0.1
  p1_distancesafe(15) = 7 / 12# / 0.1
  ...
  p1_distancemid(6) = 0#
  p1_distancemid(7) = 0#
  p1_distancemid(8) = 11 / 15# / 0.12
  p1_distancemid(9) = 0.5 / 0.12
  p1_distancemid(10) = 1 / 3# / 0.12
  p1_distancemid(11) = 1 / 8# / 0.12
  p1_distancemid(12) = 0.3 / 0.16
  p1_distancemid(13) = 1 / 3# / 0.2
  p1_distancemid(14) = 0.25 / 0.35
  p1_distancemid(15) = 0#
  ...
  p1_distanceslow(8) = 0#
  p1_distanceslow(9) = 0#
  p1_distanceslow(10) = 1 / 3# / 0.2
  p1_distanceslow(11) = 3 / 8# / 0.2
  p1_distanceslow(12) = 0.6 / 0.2
  p1_distanceslow(13) = 9 / 12# / 0.2
  p1_distanceslow(14) = 0.75 / 0.2
  p1_distanceslow(15) = 7 / 12# / 0.1
  
  p2_turnline_points00(0) = 30
  p2_turnline_points01(0) = 50
  p2_turnline_points10(0) = 80
  p2_turnline_points11(0) = 30
  p2_turnline_points00(1) = 30
  p2_turnline_points01(1) = 50
  p2_turnline_points10(1) = 80
  p2_turnline_points11(1) = 30
  p2_turnline_points00(2) = 30
  p2_turnline_points01(2) = 50
  p2_turnline_points10(2) = 80
  p2_turnline_points11(2) = 30
  p2_turnline_points00(3) = 30
  p2_turnline_points01(3) = 50
  p2_turnline_points10(3) = 80
  p2_turnline_points11(3) = 30
  p2_turnline_points00(4) = 30
  p2_turnline_points01(4) = 60
  p2_turnline_points10(4) = 100
  p2_turnline_points11(4) = 30
  p2_turnline_points00(5) = 30
  p2_turnline_points01(5) = 50
  p2_turnline_points10(5) = 100
  p2_turnline_points11(5) = 30
  p2_turnline_points00(6) = 30
  p2_turnline_points01(6) = 50
  p2_turnline_points10(6) = 110
  p2_turnline_points11(6) = 30
  p2_turnline_points00(7) = 80
  p2_turnline_points01(7) = 55
  p2_turnline_points10(7) = 120
  p2_turnline_points11(7) = 30
  p2_turnline_points00(8) = 30
  p2_turnline_points01(8) = 70
  p2_turnline_points10(8) = 120
  p2_turnline_points11(8) = 30
  p2_turnline_points00(9) = 30 ' compartment 9
  p2_turnline_points01(9) = 70 ' compartment 9
  p2_turnline_points10(9) = 120 ' compartment 9
  p2_turnline_points11(9) = 30 ' compartment 9
  p2_turnline_points00(10) = 30
  p2_turnline_points01(10) = 70
  p2_turnline_points10(10) = 120
  p2_turnline_points11(10) = 30
  p2_turnline_points00(11) = 30
  p2_turnline_points01(11) = 70
  p2_turnline_points10(11) = 120
  p2_turnline_points11(11) = 30
  p2_turnline_points00(12) = 30
  p2_turnline_points01(12) = 70
  p2_turnline_points10(12) = 120
  p2_turnline_points11(12) = 30
  p2_turnline_points00(13) = 30
  p2_turnline_points01(13) = 70
  p2_turnline_points10(13) = 120
  p2_turnline_points11(13) = 30
  p2_turnline_points00(14) = 30
  p2_turnline_points01(14) = 70
  p2_turnline_points10(14) = 120
  p2_turnline_points11(14) = 30
  p2_turnline_points00(15) = 30
  p2_turnline_points01(15) = 70
  p2_turnline_points10(15) = 120
  p2_turnline_points11(15) = 30

  p2_turnline_point1_safe(0) = 0.3 / 0.4
  p2_turnline_point1_safe(1) = 0.3 / 0.4
  p2_turnline_point1_safe(2) = 0.2 / 0.15
  p2_turnline_point1_safe(3) = 0.3 / 0.2
  p2_turnline_point1_safe(4) = 1 / 3# / 0.15
  p2_turnline_point1_safe(5) = 0.2 / 0.15
  p2_turnline_point1_safe(6) = 0.2 / 0.15
  p2_turnline_point1_safe(7) = 0
  p2_turnline_point1_safe(8) = 0
  p2_turnline_point1_safe(9) = 0
  p2_turnline_point1_safe(10) = 0
  p2_turnline_point1_safe(11) = 0
  p2_turnline_point1_safe(12) = 0
  p2_turnline_point1_safe(13) = 0
  p2_turnline_point1_safe(14) = 0
  p2_turnline_point1_safe(15) = 0
  
  p2_turnline_point1_fast(0) = 0.3 / 0.4
  p2_turnline_point1_fast(1) = 0.3 / 0.4
  p2_turnline_point1_fast(2) = 0.3 / 0.4
  p2_turnline_point1_fast(3) = 0.3 / 0.4
  p2_turnline_point1_fast(4) = 1 / 3# / 0.4
  p2_turnline_point1_fast(5) = 0.2 / 0.4
  p2_turnline_point1_fast(6) = 0.2 / 0.4
  p2_turnline_point1_fast(7) = 0
  p2_turnline_point1_fast(8) = 0
  p2_turnline_point1_fast(9) = 0
  p2_turnline_point1_fast(10) = 0
  p2_turnline_point1_fast(11) = 0
  p2_turnline_point1_fast(12) = 0
  p2_turnline_point1_fast(13) = 0
  p2_turnline_point1_fast(14) = 0
  p2_turnline_point1_fast(15) = 0
  
  p2_turnline_point1_mid(0) = 0.1 / 0.4
  p2_turnline_point1_mid(1) = 0.1 / 0.4
  p2_turnline_point1_mid(2) = 0.2 / 0.4
  p2_turnline_point1_mid(3) = 0.3 / 0.4
  p2_turnline_point1_mid(4) = 1 / 6# / 0.35
  p2_turnline_point1_mid(5) = 0.2 / 0.35
  p2_turnline_point1_mid(6) = 0.1 / 0.35
  p2_turnline_point1_mid(7) = 0
  p2_turnline_point1_mid(8) = 0
  p2_turnline_point1_mid(9) = 0
  p2_turnline_point1_mid(10) = 0
  p2_turnline_point1_mid(11) = 0
  p2_turnline_point1_mid(12) = 0
  p2_turnline_point1_mid(13) = 0
  p2_turnline_point1_mid(14) = 0
  p2_turnline_point1_mid(15) = 0
  
  p2_turnline_point2_safe(0) = 3 / 8# / 0.4
  p2_turnline_point2_safe(1) = 0.5 / 0.4
  p2_turnline_point2_safe(2) = 0.25 / 0.15
  p2_turnline_point2_safe(3) = 0.5 / 0.2
  p2_turnline_point2_safe(4) = 0.2 / 0.15
  p2_turnline_point2_safe(5) = 0.2 / 0.15
  p2_turnline_point2_safe(6) = 1 / 11# / 0.15
  p2_turnline_point2_safe(7) = 0
  p2_turnline_point2_safe(8) = 0
  p2_turnline_point2_safe(9) = 0
  p2_turnline_point2_safe(10) = 0
  p2_turnline_point2_safe(11) = 0
  p2_turnline_point2_safe(12) = 0
  p2_turnline_point2_safe(13) = 0
  p2_turnline_point2_safe(14) = 0
  p2_turnline_point2_safe(15) = 0
  
  p2_turnline_point2_fast(0) = 3 / 8# / 0.4
  p2_turnline_point2_fast(1) = 0.5 / 0.4
  p2_turnline_point2_fast(2) = 0.5 / 0.4
  p2_turnline_point2_fast(3) = 0.5 / 0.4
  p2_turnline_point2_fast(4) = 0.1 / 0.4
  p2_turnline_point2_fast(5) = 0.2 / 0.4
  p2_turnline_point2_fast(6) = 1 / 11# / 0.4
  p2_turnline_point2_fast(7) = 0
  p2_turnline_point2_fast(8) = 0
  p2_turnline_point2_fast(9) = 0
  p2_turnline_point2_fast(10) = 0
  p2_turnline_point2_fast(11) = 0
  p2_turnline_point2_fast(12) = 0
  p2_turnline_point2_fast(13) = 0
  p2_turnline_point2_fast(14) = 0
  p2_turnline_point2_fast(15) = 0
  
  p2_turnline_point2_mid(0) = 1 / 8# / 0.4
  p2_turnline_point2_mid(1) = 0.125 / 0.4
  p2_turnline_point2_mid(2) = 0.25 / 0.4
  p2_turnline_point2_mid(3) = 0.5 / 0.4
  p2_turnline_point2_mid(4) = 0.2 / 0.4
  p2_turnline_point2_mid(5) = 0.1 / 0.4
  p2_turnline_point2_mid(6) = 1 / 11# / 0.4
  p2_turnline_point2_mid(7) = 0
  p2_turnline_point2_mid(8) = 0
  p2_turnline_point2_mid(9) = 0
  p2_turnline_point2_mid(10) = 0
  p2_turnline_point2_mid(11) = 0
  p2_turnline_point2_mid(12) = 0
  p2_turnline_point2_mid(13) = 0
  p2_turnline_point2_mid(14) = 0
  p2_turnline_point2_mid(15) = 0
  
    returnfunc = (vfraction_vnitrogen(vmix_vnumber) + 0.001)
    returnfunc = (vfraction_vhelium(vmix_vnumber) + 0.001)

    arg_t = timebot '30#
    If arg_c = 11 Then
      arg_t = arg_t '30#
    End If
    gas_mixn2 = (vfraction_vnitrogen(vmix_vnumber) + 0#)
    gas_mixhe = (vfraction_vhelium(vmix_vnumber) + 0#)
    p0 = 0.79 'tissuenitrogenbot(comp) '//Initial partial pressure in compartment //njkb pass p0
'    i = partial_tissue_pressure((depthbot / 10# + 1#) * gas_mixn2, p0, arg_t, arg_c, 1)
'    pp_tissue0 = returnfunc
'    If gas = 1 Then pp_tissue0 = tissuenitrogenbot(comp) Else pp_tissue0 = tissueheliumbot(comp)
    pp_tissue0 = tissuenitrogenbot(comp)

    i = b_calc(pp_tissue0, depthbot, 1, arg_c) 'time, depth, n2, compartment
    ans_N2 = returnfunc
    
'    p0 = 0#         'tissuenitrogenbot(comp) '//Initial partial pressure in compartment //njkb pass p0
'    i = partial_tissue_pressure((depthbot / 10# + 1#) * gas_mixhe, p0, arg_t, arg_c, 2)
'    pp_tissue0 = returnfunc
'    If gas = 1 Then pp_tissue0 = tissuenitrogenbot(comp) Else pp_tissue0 = tissueheliumbot(comp)
    pp_tissue0 = tissueheliumbot(comp)
    i = b_calc(pp_tissue0, depthbot, 2, arg_c) 'time, depth, he, compartment
    ans_He = returnfunc
    ans_tot = ans_N2 * gas_mixn2 / (gas_mixn2 + gas_mixhe) + ans_He * gas_mixhe / (gas_mixn2 + gas_mixhe)
...
'    End If
    returnfunc = ans_tot
    If arg_c = 11 Then
'      MsgBox "return: " + Format(returnfunc, "#0.00") + vbCrLf + "comp: " + Format(arg_c, "#0") + vbCrLf + "n2: " + Format(gas_mixn2, "#0.00") + vbCrLf + "he: " + Format(gas_mixhe, "#0.00") + vbCrLf + "depth: " + Format(depthbot, "#0.00") + vbCrLf + "time: " + Format(arg_t, "#0.00")
    End If

End Function


'Function solve_lineqn() 'point1,float* point2,float* coeffs)
'Dim i
'    coeffs(0) = (point2(1) - point1(1)) / (point2(0) - point1(0))
'    coeffs(1) = coeffs(0) * (-point1(0)) + point1(1)
'
'End Function
    
Function coefficient_calc(minf As Double, pinf As Double, bturn00 As Double, bturn01 As Double, bturn10 As Double, bturn11 As Double) 'bturn not array rtn not address
Dim i
Dim depth1, turn1, depth2, turn2, gamma, delta, alpha, beta, s1, s2, S
    depth1 = bturn00
    turn1 = bturn10
    depth2 = bturn01
    turn2 = bturn11
    ...
    s1 = inv_tanh((turn2 - delta) / gamma)
    s1 = returnfunc
    s2 = inv_tanh((turn1 - delta) / gamma)
    s2 = returnfunc

    S = s2 / s1

    alpha = (depth1 - depth2 * S) / (1 - S)
    beta = s1 / (depth2 - alpha)
...
Function inv_tanh(val As Double)
Dim i

    If (val > 1#) Then MsgBox "Error in calculating arctanh -  |argument| > 1."
    If (val < -1#) Then MsgBox "Error in calculating arctanh -  |argument| < -1."

    returnfunc = Log((1 + val) / (1 - val)) / 2#

End Function

Function tanh(val As Double)
  If val > 700 Then val = 700
  If Exp(val) = 0 Then
    returnfunc = 0
  Else
    returnfunc = (Exp(val) - Exp(-val)) / (Exp(val) + Exp(-val))
  End If
End Function



Function b_calc(pp_tissue As Double, depth As Double, gas As Integer, comp As Integer)
Dim i
Dim bmin_fact As Double
Dim bmax_fact As Double
Dim p2_turnline_point1(2) As Double, p2_turnline_point2(2) As Double
Dim p2_coeffs(2) As Double
Dim bmin_minf As Double
Dim bmin_pinf As Double
Dim bmin_turn0(2) As Double
Dim bmin_turn1(2) As Double
Dim p1_minf As Double
Dim p1_pinf As Double
Dim p1_turn0(2) As Double
Dim p1_turn1(2) As Double
Dim p2_minf As Double
Dim p2_pinf As Double
Dim p2_turn0(2) As Double
Dim p2_turn1(2) As Double
Dim cp1(4) As Double
Dim cp2(4) As Double
Dim b_turn0(2) As Double
Dim b_turn1(2) As Double
Dim bmin1 As Double
Dim p1 As Double
Dim p2 As Double
Dim p1_point As Double
Dim p1_distance As Double
Dim p2_point As Double
Dim p1_point_turn As Double
Dim p0 As Double
Dim pp_p1 As Double
Dim pp_p2 As Double
Dim b As Double

'    //Calculate bmin and bmax given safety parameters
    'vhmx_safe_factor = 0.9 '1.1
    'vhmx_maxd_factor = 0.9 '1.1
    'vhmx_stop_factor = 0.9 '1.1
    'vhmx_mid_factor = 0.9 '1.1
    vhmx_maxd_factor = 1# + 0.01 * CDbl(vhmx_text(0).Text)
    vhmx_mid_factor = 1# + 0.01 * CDbl(vhmx_text(1).Text)
    vhmx_stop_factor = 1# + 0.01 * CDbl(vhmx_text(2).Text)
    vhmx_safe_factor = 1# + 0.01 * CDbl(vhmx_text(3).Text)
    'vhmx_safe_factor = 1.1
    'vhmx_maxd_factor = 1.1
    'vhmx_stop_factor = 1.1
    'vhmx_mid_factor = 1.1
    If gas = 1 Then
      bmin_fact = bn2(comp) * vhmx_safe_factor * (1 + bmin_fast(comp) * (vhmx_maxd_factor - 1)) * (1 + bmin_mid(comp) * (vhmx_mid_factor - 1)) * (1 + bmin_slow(comp) * (vhmx_stop_factor - 1))
      bmax_fact = bn2max(comp) * vhmx_safe_factor * (1 + bmax_fast(comp) * (vhmx_maxd_factor - 1)) * (1 + bmax_mid(comp) * (vhmx_mid_factor - 1)) * (1 + bmax_slow(comp) * (vhmx_stop_factor - 1))
    Else
      bmin_fact = bhe(comp) * vhmx_safe_factor * (1 + bmin_fast(comp) * (vhmx_maxd_factor - 1)) * (1 + bmin_mid(comp) * (vhmx_mid_factor - 1)) * (1 + bmin_slow(comp) * (vhmx_stop_factor - 1))
      bmax_fact = bhemax(comp) * vhmx_safe_factor * (1 + bmax_fast(comp) * (vhmx_maxd_factor - 1)) * (1 + bmax_mid(comp) * (vhmx_mid_factor - 1)) * (1 + bmax_slow(comp) * (vhmx_stop_factor - 1))
    End If

    If bmin_fact <= bmax_fact Then
        If bmin_fact > (0.99 - 0.00001) Then bmin_fact = 0.99 - 0.00001
        If bmax_fact > 0.99 Then bmax_fact = 0.99
    Else
        If bmin_fact > 0.99 Then bmin_fact = 0.99
        If bmax_fact > (0.99 - 0.00001) Then bmax_fact = 0.99 - 0.00001
    End If

'    //Adjust the second turning point according to safety parameters
    p2_turnline_point1(0) = p2_turnline_points00(comp)
'    //adjustment to time component of q1
    p2_turnline_point1(1) = p2_turnline_points01(comp) * (1 - p2_turnline_point1_safe(comp) * (vhmx_safe_factor - 1)) * (1 - p2_turnline_point1_fast(comp) * (vhmx_maxd_factor - 1)) * (1 - p2_turnline_point1_mid(comp) * (vhmx_mid_factor - 1))

'    //adjustment to depth component of q2
    p2_turnline_point2(0) = p2_turnline_points10(comp) * (1 - p2_turnline_point2_safe(comp) * (vhmx_safe_factor - 1)) * (1 - p2_turnline_point2_fast(comp) * (vhmx_maxd_factor - 1)) * (1 - p2_turnline_point2_mid(comp) * (vhmx_mid_factor - 1))
    p2_turnline_point2(1) = p2_turnline_points11(comp)

    If p2_turnline_point1(1) <= 30 Then p2_turnline_point1(1) = 30.00001
    If p2_turnline_point2(0) <= 30 Then p2_turnline_point2(0) = 30.00001

'    //Adjust first turning point in time according to safety parameters
    p1_point = p1_point_default(comp) * (1 - p1_pointsafe(comp) * (vhmx_safe_factor - 1)) * (1 - p1_pointmid(comp) * (vhmx_mid_factor - 1)) * (1 - p1_pointslow(comp) * (vhmx_stop_factor - 1))
    If p1_point < 30# Then
      p1_point = 30#
    Else
      If p1_point >= p2_turnline_point2(0) Then p1_point = p2_turnline_point2(0) - 0.00001
    End If

'    //Used for compartments 9-16. Distance between p2 turn line and p1.
'    //Used to find the first turn line in time according to safety parameters
    p1_distance = p1_distancedefault(comp) * (1 - p1_distancesafe(comp) * (vhmx_safe_factor - 1)) * (1 - p1_distancemid(comp) * (vhmx_mid_factor - 1)) * (1 - p1_distanceslow(comp) * (vhmx_stop_factor - 1))
    If p1_distance <= 0# Then p1_distance = 0.00001

'    i = solve_lineqn(p2_turnline_point1, p2_turnline_point2, p2_coeffs)

'Function solve_lineqn(point1,point2,coeffs)
    p2_coeffs(0) = (p2_turnline_point2(1) - p2_turnline_point1(1)) / (p2_turnline_point2(0) - p2_turnline_point1(0))
    p2_coeffs(1) = p2_coeffs(0) * (-p2_turnline_point1(0)) + p2_turnline_point1(1)

    bmin_minf = bmin_fact
    bmin_pinf = bmax_fact
    bmin_turn0(0) = p1_point
    bmin_turn0(1) = p2_turnline_point2(0)
    bmin_turn1(0) = bmin_fact + (bmax_fact - bmin_fact) / 150
    bmin_turn1(1) = bmax_fact - (bmax_fact - bmin_fact) / 70


    i = coefficient_calc(bmin_minf, bmin_pinf, bmin_turn0(0), bmin_turn0(1), bmin_turn1(0), bmin_turn1(1))

'    //This is the initial value for b at time 0, given depth.
    i = tanh(cbmin(1) * (depth - cbmin(0)))
    bmin1 = cbmin(2) * returnfunc + cbmin(3)

'    //Second turning point in time, given depth.
    p2_point = p2_coeffs(0) * depth + p2_coeffs(1)
    If p2_point <= 30 Then
      p2_point = 30.00001
    Else
      p2_point = p2_point
...
'    //First turing point in time.
'    //Compartments 1-8 have a first turning point = 30min
    p1_point_turn = p2_point - p1_distance
    If p1_point_turn < 30# Then
      p1_point_turn = 30#
    Else
      p1_point_turn = p1_point_turn
    End If

    
    If gas = 1 Then
      p0 = 0.79 'tissuenitrogenbot(comp) '//Initial partial pressure in compartment //njkb pass p0
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vnitrogen(vmix_vnumber) + 0.001), p0, p1_point_turn, comp, 1)
      pp_p1 = returnfunc
'      pp_p1 = tissuenitrogenbot(comp)
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vnitrogen(vmix_vnumber) + 0.001), p0, p2_point, comp, 1)
      pp_p2 = returnfunc
'      pp_p2 = tissueheliumbot(comp)
    Else
      p0 = 0.0001 'tissueheliumbot(comp) '//Initial partial pressure in compartment //njkb pass p0
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vhelium(vmix_vnumber) + 0.001), p0, p1_point_turn, comp, 2)
      pp_p1 = returnfunc
'      pp_p1 = tissuenitrogenbot(comp)
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vhelium(vmix_vnumber) + 0.001), p0, p2_point, comp, 2)
      pp_p2 = returnfunc
'      pp_p2 = tissueheliumbot(comp)
    End If

    If pp_p1 = pp_p2 Then pp_p2 = pp_p1 + 0.00001

    b_turn0(0) = pp_p1
    b_turn0(1) = pp_p2
    b_turn1(0) = bmin1 + (bmax_fact - bmin1) / 100
    b_turn1(1) = bmax_fact - (bmax_fact - bmin1) / 100

    i = coefficient_calc(bmin1, bmax_fact, b_turn0(0), b_turn0(1), b_turn1(0), b_turn1(1))

    i = tanh(cbbb(1) * (pp_tissue - cbbb(0)))
    returnfunc = cbbb(2) * returnfunc + cbbb(3)

End Function


Function partial_tissue_pressure(pa As Double, p0 As Double, time As Double, compartment As Integer, gas As Integer)
Dim i

    If gas = 1 Then
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / (vnitrogen_half_vtime(compartment + 1) + 0.001)))
    Else
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / (vhelium_half_vtime(compartment + 1) + 0.001)))
    End If
