...
'//for all compartments later in calculation
  ...
  p2_turnline_point2_mid(15) = 0
    ...
    ans_He = returnfunc
    gas_mixn2 = tissuenitrogenbot(compartment)
    gas_mixhe = tissueheliumbot(compartment)
...
Function b_calc(time As Double, depth As Double, gas As Integer, comp As Integer)
Dim i
...
'    //This is the initial value for b at time 0, given depth.
    i = tanh(cbmin(1) * (depth - cbmin(0))) + cbmin(3)
    bmin1 = cbmin(2) * returnfunc

'    //Second turning point in time, given depth.
    p2_point = p2_coeffs(0) * depth + p2_coeffs(1)
    If p2_point <= 30 Then p2_point = 30.00001

'    //First turing point in time.
'    //Compartments 1-8 have a first turning point = 30min
    p1_point_turn = p2_point - p1_distance
    If p1_point_turn < 30# Then p1_point_turn = 30#

    
    If gas = 1 Then
      p0 = 0.79 'tissuenitrogenbot(comp) '//Initial partial pressure in compartment //njkb pass p0
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vnitrogen(compartment + 1) + 0.001), p0, p1_point_turn, comp, 1)
      pp_p1 = returnfunc
      pp_p1 = tissuenitrogenbot(comp)
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vnitrogen(compartment + 1) + 0.001), p0, p2_point, comp, 1)
      pp_p2 = returnfunc
      pp_p2 = tissueheliumbot(comp)
    Else
      p0 = 0.0001 'tissueheliumbot(comp) '//Initial partial pressure in compartment //njkb pass p0
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vhelium(compartment + 1) + 0.001), p0, p1_point_turn, comp, 2)
      pp_p1 = returnfunc
      pp_p1 = tissuenitrogenbot(comp)
      i = partial_tissue_pressure((depth / 10# + 1#) * (vfraction_vhelium(compartment + 1) + 0.001), p0, p2_point, comp, 2)
      pp_p2 = returnfunc
      pp_p2 = tissueheliumbot(comp)
    End If
...
'  pp_tissue = returnfunc
    ...
    i = tanh(cbbb(1) * (pp_tissue - cbbb(0))) + cbbb(3)
    returnfunc = cbbb(2) * returnfunc
...
Dim i
    ...
    If gas = 1 Then
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / (vfraction_vnitrogen(compartment + 1) + 0.001)))
    Else
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / (vfraction_vhelium(compartment + 1) + 0.001)))
    End If
