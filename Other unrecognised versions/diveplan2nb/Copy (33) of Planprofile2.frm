...
'    //Compartments 1-8 have a first turning point = 30min
    ...
    If gas = 1 Then
      p0 = tissuenitrogenbot(comp) '//Initial partial pressure in compartment //njkb pass p0
    Else
      p0 = tissueheliumbot(comp) '//Initial partial pressure in compartment //njkb pass p0
    End If
    i = partial_tissue_pressure((depth / 10# + 1#) * gas_mixn2, p0, p1_point_turn, comp, 1)
    pp_p1 = returnfunc
    i = partial_tissue_pressure((depth / 10# + 1#) * gas_mixhe, p0, p2_point, comp, 2)
    pp_p2 = returnfunc
...
Dim i
    ...
    If gas = 1 Then
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / tissuenitrogenbot(compartment)))
    Else
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / tissueheliumbot(compartment)))
    End If
