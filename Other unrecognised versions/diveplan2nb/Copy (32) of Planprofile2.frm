...
Function jpb_main() ' //pp_t
Dim i
Dim ans_N2 As Integer
Dim ans_He As Integer
...
'//for all compartments later in calculation
  ...
  p1_distanceslow(8) = 0#
  p1_distanceslow(9) = 0#
  p1_distanceslow(10) = 1 / 3# / 0
  p1_distanceslow(11) = 2
  p1_distanceslow(12) = 3 / 8# / 0
  p1_distanceslow(13) = 2
  p1_distanceslow(14) = 0.6 / 0.2
  p1_distanceslow(15) = 9 / 12# / 0.2
  p1_distanceslow(16) = 0.75 / 0.2
  p1_distanceslow(17) = 7 / 12# / 0.1
  ...
  p2_turnline_point2_mid(15) = 0
    ...
    i = b_calc(arg_t, arg_d, 1, arg_c) 'time, depth, n2, compartment
    ans_N2 = returnfunc
    i = b_calc(arg_t, arg_d, 2, arg_c) 'time, depth, he, compartment
    ans_He = returnfunc
...
'    //Calculate bmin and bmax given safety parameters
    If gas = 1 Then
      bmin_fact = bn2(comp - 1) '*vhmx_safe_factor*(1+bmin_fast(comp)*(vhmx_maxd_factor-1))*(1+bmin_mid(comp)*(vhmx_mid_factor-1))*(1+bmin_slow(comp)*(vhmx_stop_factor-1))
      bmax_fact = bn2max(comp - 1) '*vhmx_safe_factor*(1+bmax_fast(comp)*(vhmx_maxd_factor-1))*(1+bmax_mid(comp)*(vhmx_mid_factor-1))*(1+bmax_slow(comp)*(vhmx_stop_factor-1))
    Else
      bmin_fact = bhe(comp - 1) '*vhmx_safe_factor*(1+bmin_fast(comp)*(vhmx_maxd_factor-1))*(1+bmin_mid(comp)*(vhmx_mid_factor-1))*(1+bmin_slow(comp)*(vhmx_stop_factor-1))
      bmax_fact = bhemax(comp - 1) '*vhmx_safe_factor*(1+bmax_fast(comp)*(vhmx_maxd_factor-1))*(1+bmax_mid(comp)*(vhmx_mid_factor-1))*(1+bmax_slow(comp)*(vhmx_stop_factor-1))
    End If
...
'    //Compartments 1-8 have a first turning point = 30min
    ...
    If gas = 1 Then
      p0 = tissuen2(comp) '//Initial partial pressure in compartment //njkb pass p0
    Else
      p0 = tissuehe(comp) '//Initial partial pressure in compartment //njkb pass p0
    End If
'    //Partial pressures of tissue at turning points given depth and time
    i = partial_tissue_pressure((depth / 10# + 1#) * gas_mix(gas), p0, p1_point_turn, comp, gas)
    pp_p1 = returnfunc
    i = partial_tissue_pressure((depth / 10# + 1#) * gas_mix(gas), p0, p2_point, comp, gas)
    pp_p2 = returnfunc
...
'  pp_tissue = returnfunc
    ...
    i = tanh(cb(1) * (pp_tissue - cb(0))) + cb(3)
    b = cb(2) * returnfunc
...
Dim i
    ...
    If gas = 1 Then
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / vnitrogen_half_vtime(compartment)))
    Else
      returnfunc = p0 + (pa - p0) * (1 - Exp(-Log(2) * time / vhelium_half_vtime(compartment)))
    End If
