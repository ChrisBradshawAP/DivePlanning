...
'//for all compartments later in calculation
  ...
  p2_turnline_point2_mid(15) = 0
    ...
    arg_t = 30#
    i = b_calc(arg_t, depthbot, 1, arg_c) 'time, depth, n2, compartment
    ans_N2 = returnfunc
    i = b_calc(arg_t, depthbot, 2, arg_c) 'time, depth, he, compartment
    ans_He = returnfunc
    gas_mixn2 = (vfraction_vnitrogen(vmix_vnumber) + 0.001)
    gas_mixhe = (vfraction_vhelium(vmix_vnumber) + 0.001)
...
'  partial_tissue_pressure((depth/10.0+1.0)*gas_mix(gas),p0,time,comp,gas)
'  pp_tissue = returnfunc

   If gas = 1 Then pp_tissue = tissuenitrogenbot(comp) Else pp_tissue = tissueheliumbot(comp)
