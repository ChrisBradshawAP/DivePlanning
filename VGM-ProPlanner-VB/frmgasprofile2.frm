...
Case vbYes
    ...
    While RS.EOF = False
       If CInt(i) < 10 Then
          ...
          txtmaxd(i) = CInt(RS("gasmaxopdepth")) / 10
...
Case "NP"
  ...
  newserialno = CInt(tempserialno) + 1
...
Case "NSP"
 ...
 Case "NPP"
  ...
  newserialno = CInt(tempserialno) + 1
  tempserialno = CInt(tempserialno) + 1
  'newserialno = "1" '"000000001" 'Cint(tempserialno) + 1
  'tempserialno = "0" '"000000002" 'Cint(tempserialno) + 1
  lengthsn = Len(tempserialno)
  Select Case lengthsn
  Case 1
     tempserialno = "TP0000000" & tempserialno
...
Private Sub validateoxygen()
   If CInt(txtoxygen(p).Text) >= 0 And CInt(txtoxygen(p).Text) <= 100 And ((CInt(txtoxygen(p).Text) + CInt(txthelium(p).Text)) < 101) Then
     ...
     If Cbogasused(p).Text = "5 - Deco Closed Circuit" And (CDbl(temptextpo2) <> CDbl(txtppo2(p).Text)) Then
        ans = MsgBox("Do you really want to load the factory default setting ?", vbYesNo + vbCritical + vbDefaultButton2, "Comfirmation")
        Select Case ans
           Case vbYes
              txtppo2(p).Enabled = True
...
Private Sub validatemaxdepth()
   If CInt(txtmaxd(p).Text) >= 0 And CInt(txtmaxd(p).Text) <= 1000 Then
     temptextpo2 = (CInt(txtoxygen(p).Text) / 100) * ((CInt(txtmaxd(p).Text) / 10) + 1)
     temptextpo2 = Format(temptextpo2, "###.00")
     If Cbogasused(p).Text = "5 - Deco Closed Circuit" And (CDbl(temptextpo2) <> CDbl(txtppo2(p).Text)) Then
        ans = MsgBox("Do you really want to load the factory default setting ?", vbYesNo + vbCritical + vbDefaultButton2, "Comfirmation")
...
Private Sub validatepo2()
  If CDbl(txtppo2(p).Text) <= 0.14 Then
    MsgBox "PO2 value out of range "
    txtppo2(p).Enabled = True
    txtppo2(p).SetFocus
  Else
    If CDbl(txtppo2(p).Text) > 2 Then
       MsgBox "PO2 value out of range "
       txtppo2(p).Enabled = True
       txtppo2(p).SetFocus
       SendKeys "{HOME}+{END}"
